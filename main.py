# ===== REQUIREMENTS =====
# numpy, pandas, matplotlib, scipy, scikit-learn, cvxpy, torch, tqdm, cptopt

# ===== TASK 0: IMPORTS AND SEEDS =====
...
def load_asset_subset(csv_path, asset_names):
    ...
    assert df.notna().all().all(), f"NaNs remain after slicing!"
...
# ===== TASK 4: ADAM AND LBFGS OPTIMIZERS =====
...
class AdamOptimizer:
    ...
    def optimize(self, R, initial_weights=None, **kwargs):
        ...
        x0 = np.log(w0 + 1e-8)
        x = torch.tensor(x0, device=DEVICE, dtype=torch.float64, requires_grad=True)
        x = nn.Parameter(x)
        ...
        for i in range(self.max_iter):
            ...
            self._weights_history.append(w_np)
            self._wall_time.append(time.time())
            if time.time() - t0 > self.max_time:
                break

        w_final = torch.softmax(x, dim=0)
        self._weights = w_final.detach().cpu().numpy()


class LBFGSOptimizer:
    def __init__(self, utility_obj, max_iter=5_000, max_time=600):
        self._utility = copy.deepcopy(utility_obj)
        CPTUtilityGA.convert_to_class(self._utility)
        self.max_iter = max_iter
        self.max_time = max_time
...
# ===== HELPER: evaluate CPT utility (scalar) =====
def cpt_eval(w, R):
...
if __name__ == '__main__':
    ...
    # Local optimum: mask ball of radius 0.1 around w_star
    ...
    # --- Figure 2a: MV Frontier on CPT Surface ---
    mv_opt = MeanVarianceFrontierOptimizer(utility)
    mv_opt.optimize(R_toy, samples=100)
    mv_frontier_weights = mv_opt._weights_history  # (100, 3)
    w_mv = mv_opt.weights
    ...
    ax.plot(fw[:, 0], fw[:, 1], '-', color='red', linewidth=3, label='MV Frontier',
            zorder=5)
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig('figures/fig2a_mv_frontier.png', dpi=150)
    plt.close()
    print("Saved figures/fig2a_mv_frontier.png")

    # --- Figure 2b: MV Utility Surface ---
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig('figures/fig2b_mv_surface.png', dpi=150)
    plt.close()
    print("Saved figures/fig2b_mv_surface.png")

    # --- Figure 3 (a,b,c): Convergence Trajectories ---
    ...
    # Subsample for MM/CC trajectory plots (SCS solver is very slow on N>100)
    R_traj = R_toy[::2]  # ~115 rows, much faster for CVXPY solves
    print(f"Using subsampled R_traj ({R_traj.shape[0]} rows) for MM/CC trajectories")

    # MM trajectories
    print("Running MM convergence...")
    mm_trajs = OrderedDict()
    for name, w0 in starts.items():
        mm = MinorizationMaximizationOptimizer(utility, max_iter=10)
        mm.optimize(R_traj, initial_weights=w0.copy(), max_time=30)
        mm_trajs[name] = mm._weights_history
    fig, ax = plt.subplots(figsize=(8, 6))
    plot_trajectories(ax, mm_trajs, 'MM Convergence')
    plt.tight_layout()
    plt.savefig('figures/fig3a_mm.png', dpi=150)
    plt.close()
    print("Saved figures/fig3a_mm.png")

    # CC trajectories
    print("Running CC convergence...")
    cc_trajs = OrderedDict()
    for name, w0 in starts.items():
        cc = ConvexConcaveOptimizer(utility, max_iter=10)
        cc.optimize(R_traj, initial_weights=w0.copy(), max_time=30)
        cc_trajs[name] = cc._weights_history
    fig, ax = plt.subplots(figsize=(8, 6))
    plot_trajectories(ax, cc_trajs, 'CC Convergence')
    plt.tight_layout()
    plt.savefig('figures/fig3b_cc.png', dpi=150)
    plt.close()
    print("Saved figures/fig3b_cc.png")
    ...
    plt.tight_layout()
    plt.savefig('figures/fig3c_ga.png', dpi=150)
    plt.close()
    print("Saved figures/fig3c_ga.png")

    print(f"Task 2 elapsed: {time.time()-t2_start:.1f}s")

    # ===== TASK 3: MULTI-ASSET EXAMPLE =====
    print("\n" + "="*60)
    print("TASK 3: MULTI-ASSET EXAMPLE")
    print("="*60)
    t3_start = time.time()
    ...
    # --- Figure 4: SSPW Boxplot ---
    print("Running SSPW rolling backtest (window=60)...")
    window = 60
    n_steps = R_full.shape[0] - window

    utility_no_reweight = CPTUtility(gamma_pos=GAMMA_POS, gamma_neg=GAMMA_NEG,
                                     delta_pos=0.99, delta_neg=0.99)
    utility_no_loss_av = CPTUtility(gamma_pos=GAMMA_NEG, gamma_neg=GAMMA_NEG,
                                    delta_pos=DELTA_POS, delta_neg=DELTA_NEG)

    sspw_methods = OrderedDict([
        ('MV', {'cls': MeanVarianceFrontierOptimizer, 'util': utility, 'kw': {}}),
        ('GA', {'cls': GradientOptimizer, 'util': utility, 'kw': {'max_iter': 5000}}),
        ('CC', {'cls': ConvexConcaveOptimizer, 'util': utility, 'kw': {'max_iter': 5}}),
        ('CC-no-reweight', {'cls': ConvexConcaveOptimizer, 'util': utility_no_reweight, 'kw': {'max_iter': 5}}),
        ('CC-no-loss-av', {'cls': ConvexConcaveOptimizer, 'util': utility_no_loss_av, 'kw': {'max_iter': 5}}),
    ])

    sspw_records = {name: [] for name in sspw_methods}
    prev_weights = {name: np.ones(n_full)/n_full for name in sspw_methods}

    for t in tqdm(range(n_steps), desc="SSPW backtest"):
        train_R = R_full[t:t+window]
        for name, cfg in sspw_methods.items():
            opt = cfg['cls'](cfg['util'], **cfg['kw'])
            w0 = prev_weights[name]
            try:
                if name == 'MV':
                    ...
                    opt.optimize(train_R, initial_weights=w0, max_time=60)
                w = opt.weights
            except Exception as e:
                w = w0  # fallback to previous weights on error
            prev_weights[name] = w
            sspw = np.sum((w - np.ones(n_full)/n_full)**2)
            sspw_records[name].append(sspw)

    fig, ax = plt.subplots(figsize=(10, 6))
    bp_data = [sspw_records[n] for n in sspw_methods]
    ax.boxplot(bp_data, labels=list(sspw_methods.keys()))
    ax.set_ylabel('SSPW')
    ax.set_title('Sum of Squared Portfolio Weight Deviations')
    plt.xticks(rotation=30)
    plt.tight_layout()
    plt.savefig('figures/fig4_sspw.png', dpi=150)
    plt.close()
    print("Saved figures/fig4_sspw.png")

    # --- Figures 5a/5b: Wall-time vs Utility ---
    def run_walltime_experiment(R, w0, label, savepath):
        ...
        wh = ga_o._weights_history
        wt = ga_o.wall_time
        # subsample for efficiency
        step = max(1, len(wh) // 500)
        ga_times = wt[::step].tolist()
        ga_utils = [cpt_eval(wh[i], R) for i in range(0, len(wh), step)]
        results['GA'] = {'times': ga_times[:len(ga_utils)], 'utils': ga_utils}

        # GA-Adam
        adam_o = AdamOptimizer(utility, lr=1e-2, max_iter=100_000, max_time=60)
        adam_o.optimize(R, initial_weights=w0.copy())
        wh = adam_o._weights_history
        wt = adam_o.wall_time
        step = max(1, len(wh) // 500)
        adam_times = wt[::step].tolist()
        adam_utils = [cpt_eval(wh[i], R) for i in range(0, len(wh), step)]
        results['Adam'] = {'times': adam_times[:len(adam_utils)], 'utils': adam_utils}

        # GA-LBFGS
        lbfgs_o = LBFGSOptimizer(utility, max_iter=5_000, max_time=60)
        lbfgs_o.optimize(R, initial_weights=w0.copy())
        wh = lbfgs_o._weights_history
        wt = lbfgs_o.wall_time
        if len(wh) > 0:
            lbfgs_utils = [cpt_eval(wh[i], R) for i in range(len(wh))]
            results['LBFGS'] = {'times': wt.tolist()[:len(lbfgs_utils)], 'utils': lbfgs_utils}

        # Plot
        best_util = max(max(v['utils']) for v in results.values())
        fig, ax = plt.subplots(figsize=(10, 6))
        markers = {'MV': 's', 'MM': 'o', 'CC': '^', 'GA': 'v', 'Adam': 'D', 'LBFGS': 'P'}
        for name, data in results.items():
            t_arr = np.array(data['times'])
            t_arr = np.maximum(t_arr, 1e-4)  # avoid log(0)
    ...
    print(f"Task 3 elapsed: {time.time()-t3_start:.1f}s")

    # ===== TASK 5: ROLLING WINDOW BACKTEST =====
    print("\n" + "="*60)
    print("TASK 5: ROLLING WINDOW BACKTEST")
    print("="*60)
    t5_start = time.time()
    ...
    def rolling_backtest(R, methods_dict, window=60):
        ...
        for name in methods_dict:
            ...
            results[name] = {
                'ann_return': ann_return, 'ann_sharpe': ann_sharpe,
                'max_drawdown': max_dd, 'mean_is_cpt': mean_is_cpt,
                'realized_ret': rets, 'wealth': wealth
            }
        return results
    ...
    # Plot 2: Monthly realized returns
    ...
    print(f"Task 5 elapsed: {time.time()-t5_start:.1f}s")

    # ===== TASK 6: SCALING TEST =====
    print("\n" + "="*60)
    print("TASK 6: SCALING TEST")
    print("="*60)
    t6_start = time.time()
    ...
    ga_wh = ga_synth._weights_history
    ga_wt = ga_synth.wall_time
    step_g = max(1, len(ga_wh) // 200)
    ga_synth_times = ga_wt[::step_g]
    ga_synth_utils = [cpt_eval(ga_wh[i], R_synth) for i in tqdm(range(0, len(ga_wh), step_g),
                                                                  desc="GA eval")]
    ...
    adam_wh = adam_synth._weights_history
    adam_wt = adam_synth.wall_time
    step_a = max(1, len(adam_wh) // 200)
    adam_synth_times = adam_wt[::step_a]
    adam_synth_utils = [cpt_eval(adam_wh[i], R_synth) for i in tqdm(range(0, len(adam_wh), step_a),
                                                                     desc="Adam eval")]

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(ga_synth_times[:len(ga_synth_utils)], ga_synth_utils, '-o',
            label='GA-vanilla', markersize=2)
    ax.plot(adam_synth_times[:len(adam_synth_utils)], adam_synth_utils, '-D',
            label='GA-Adam', markersize=2)
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig('figures/fig6_scaling.png', dpi=150)
    plt.close()
    print("Saved figures/fig6_scaling.png")

    print(f"Task 6 elapsed: {time.time()-t6_start:.1f}s")
    ...
    # Collect final utilities and wall-times from Task 3 results (equal-weight start)
